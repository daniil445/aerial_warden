...
{
    connect(&m_timer,&QTimer::timeout,this,&VideoRecorder::grabFrame);
}
...
bool VideoRecorder::buildPipeline(int w,int h,int fps)
{
    ...
    {
        qDebug()<<"Failed create gstreamer elements";
        return false;
    }

    QString filename=
        QCoreApplication::applicationDirPath()+"/record_"+
...
bool VideoRecorder::start(QWidget *window,int fps)
{
    if(m_running)
        return false;
    ...
    if(!buildPipeline(s.width(),s.height(),fps))
        return false;
